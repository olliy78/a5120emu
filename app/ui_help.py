...
**Beide Oberflächen benutzen dieses Fenster** (Emulator und DiskTool); jede
bringt nur ihre eigene `.md` und ihren Fenstertitel mit.
...
#: Das Handbuch des DiskTool — neben dem Paket-Baum, nicht in `doc/` (das ist
#: nicht dabei).  Der Emulator reicht seines über den Bauwert ``pfad`` herein.
HANDBUCH = Path(__file__).resolve().parent / "disktool" / "help" / "handbuch.md"
...
class HelpWindow(QWidget):
    ...
    def __init__(self, parent=None, titel: str = "Handbuch",
                 pfad: Path = HANDBUCH):
        ...
        self.pfad = Path(pfad)
        self.text = lade_handbuch(self.pfad)

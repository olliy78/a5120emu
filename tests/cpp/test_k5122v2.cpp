/**
 * @file test_k5122v2.cpp
 * @brief GoogleTests für K5122v2 – neue, formatagnostische Floppy-Controller-Karte.
 *
...
class K5122v2Test : public ::testing::Test {
protected:
    K1520Bus  bus;
    K5122v2   card{bus};
...
TEST_F(K5122v2Test, Mount_ViaPath_DriveAktiv) {
    auto path = tmpImg1();
...
TEST_F(K5122v2Test, Mount_NichtExistierendeDatei_Fehler) {
    EXPECT_FALSE(card.mountDisk(0, "/nonexistent/disk.img", fmt1));
    EXPECT_FALSE(card.isDiskActive(0));
}

TEST_F(K5122v2Test, Unmount_LaufwerkInaktiv) {
    auto path = tmpImg1();
...
TEST_F(K5122v2Test, Mount_BereichsüberschreitungAbgelehnt) {
    auto path = tmpImg1();
...
TEST_F(K5122v2Test, DriveSelect_AlleVierLaufwerke) {
...
TEST_F(K5122v2Test, DriveSelect_StatusPortBReflectiertGewähltesLaufwerk) {
    auto path0 = tmpImg1();
    auto path1 = tmpImg1();
...
TEST_F(K5122v2Test, StatusPortB_KeineMontage_RDYLInaktiv) {
    card.ioWrite(0x18, 0xEE);   // D0 wählen
    uint8_t status = card.ioRead(0x12);
    EXPECT_NE(status & 0x01, 0) << "/RDYL=1 erwartet (kein Laufwerk montiert)";
}

TEST_F(K5122v2Test, StatusPortB_Montiert_RDYLAktiv) {
    auto path = tmpImg1();
    ASSERT_TRUE(card.mountDisk(0, path, fmt1));
...
TEST_F(K5122v2Test, StatusPortB_Schreibschutz_WPGesetzt) {
    auto path = tmpImg1();
...
/**
 * @test K5122v2Test/StreamingRead_EnthältSektordaten
 ...
 */
TEST_F(K5122v2Test, StreamingRead_EnthältSektordaten) {
    auto path = tmpImg1();
    ASSERT_TRUE(card.mountDisk(0, path, fmt1));
    card.ioWrite(0x18, 0xEE);   // D0
...
/**
 * @test K5122v2Test/StreamingRead_EnthältIDAM
 * @brief Der Strom nach /STR-Strobe enthält das IDAM-Markenbyte 0xFE.
 */
TEST_F(K5122v2Test, StreamingRead_EnthältIDAM) {
    auto path = tmpImg1();
    ASSERT_TRUE(card.mountDisk(0, path, fmt1));
    card.ioWrite(0x18, 0xEE);
    strobeRead(0);
...
/**
 * @test K5122v2Test/StreamingRead_ZyklischWrapAround
 ...
 */
TEST_F(K5122v2Test, StreamingRead_ZyklischWrapAround) {
    auto path = tmpImg1();
    ASSERT_TRUE(card.mountDisk(0, path, fmt1));
    card.ioWrite(0x18, 0xEE);
    strobeRead(0);
...
/**
 * @test K5122v2Test/MKStrobe_SyncsAufMarkenbyte
 ...
 */
TEST_F(K5122v2Test, MKStrobe_SyncsAufMarkenbyte) {
    auto path = tmpImg1();
    ASSERT_TRUE(card.mountDisk(0, path, fmt1));
    card.ioWrite(0x18, 0xEE);
...
/**
 * @test K5122v2Test/HeadLatch_Bit2_Head0_und_Head1_UnterschiedlicheDaten
 ...
 * Muster enthalten.
 */
TEST_F(K5122v2Test, HeadLatch_Bit2_Head0_und_Head1_UnterschiedlicheDaten) {
    auto path = tmpImg2();
...
/**
 * @test K5122v2Test/HeadLatch_NurAmSTRGelatcht
 ...
 */
TEST_F(K5122v2Test, HeadLatch_NurAmSTRGelatcht) {
    auto path = tmpImg2();
    ASSERT_TRUE(card.mountDisk(0, path, fmt2));
    card.ioWrite(0x18, 0xEE);
...
/**
 * @test K5122v2Test/IndexPuls_NachAusreichendCycles_HasInterrupt
 ...
 */
TEST_F(K5122v2Test, IndexPuls_NachAusreichendCycles_HasInterrupt) {
    auto path = tmpImg1();
    ASSERT_TRUE(card.mountDisk(0, path, fmt1));
    card.ioWrite(0x18, 0xEE);
...
/**
 * @test K5122v2Test/IndexPuls_NichtMontiert_KeinInterrupt
 * @brief Ohne gemountete Diskette erzeugt update() keinen Index-Puls.
 */
TEST_F(K5122v2Test, IndexPuls_NichtMontiert_KeinInterrupt) {
    card.setIEI(true);
...
/**
 * @test K5122v2Test/DaisyChain_IEITrue_IEOPassthrough
 * @brief Bei IEI=true und keinem ausstehenden Interrupt wird IEO=true durchgereicht.
 */
TEST_F(K5122v2Test, DaisyChain_IEITrue_IEOPassthrough) {
    card.setIEI(true);
...
/**
 * @test K5122v2Test/DaisyChain_IEIFalse_IEOFalse
 * @brief Bei IEI=false wird IEO=false (Chain blockiert).
 */
TEST_F(K5122v2Test, DaisyChain_IEIFalse_IEOFalse) {
    card.setIEI(false);
    EXPECT_FALSE(card.getIEO()) << "IEO sollte false sein, wenn IEI=false";
}

/**
 * @test K5122v2Test/DaisyChain_GetVector_OhneInterrupt_0xFF
 * @brief Ohne ausstehenden Interrupt gibt getVector() 0xFF zurück.
 */
TEST_F(K5122v2Test, DaisyChain_GetVector_OhneInterrupt_0xFF) {
...
/**
 * @test K5122v2Test/WriteRoundtrip_AenderungSichtbar
 ...
 */
TEST_F(K5122v2Test, WriteRoundtrip_ErsteSektorÄnderung) {
...
TEST_F(K5122v2Test, OhneLaufwerk_STRStrobeKeinAbsturz) {

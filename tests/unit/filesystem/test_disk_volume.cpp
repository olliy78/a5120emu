...
TEST(DiskVolume, GeometrieOhneProfilWirdUeberDieCpaRegelGeoeffnet) {
    const std::string pfad =
        (fs::temp_directory_path() / "k1520_dv_ohne_profil.hfe").string();
...
TEST(DiskVolume, MsDosDisketteWirdAlsSolcheGemeldet) {
    const std::string pfad = (fs::temp_directory_path() / "k1520_dv_fat.hfe").string();
...
TEST(DiskVolume, JedesKatalogformatLaesstSichAnlegenUndWiederOeffnen) {
    const std::string pfad = (fs::temp_directory_path() / "k1520_dv_alle.hfe").string();
    int geprueft = 0;

 ...
 *
 * HFE verschraenkt zwei Seiten zu je 256 B ([S0][S1]).  Bei EINSEITIGEN Medien
 * (num_sides==1) liegt die Spur KONTINUIERLICH (volle 512 B/Block) — sonst wuerde
 * man ueber das Spurende hinaus in die Folgespur lesen (Sektor-Doppelung).
 */
std::vector<uint8_t> seitenBytes(const std::vector<uint8_t>& file, size_t track_start,
    ...
    const uint32_t slot     = (num_sides == 1) ? 512u : 256u;
